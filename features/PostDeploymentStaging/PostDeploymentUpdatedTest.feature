Feature: Verify New room page features
    ...
    Scenario Outline: Check that user is able to create and schedule a session and check if no session ruinning all the buttons should be active
        ...
        Examples:
            ...
            | admin | C         | 8:00 pm    | stagingLoginUrl | 9:00 pm | Session has been scheduled |
    ...
    Scenario Outline: If we have multiple hosts for e.g Host 1 and Host 2. Host 1 has started the session the Join room button should not get disabled for the Host 2 for other session
        ...
        Examples:
            ...
            | tester.desk4+Subuser01@gmail.com | Testing12345! | B         | stagingLoginUrl | pdf  | 10:00 pm   |
...
Scenario Outline: For standard event Check if displaying exact number of "Regsitration", "Attended" and "Missed" on "Insights Page" also verify the functionality of "Date range" filter
    ...
    Then Shared resource number <resourceNumber> should displayed for all resources
    When User clicks on "People" tab
    Then For date range script correct user info <regEmail1> is displaying under people tab
    ...
    Then Message text should displaying under message tab <message>
    Examples:
        | userA | eventName | regEmail2 | regEmail1 | userCount | regEmail3 | name | slide        | regCount | missedCount | attendedCount | futureTime | regCountForNextDay | attendedCountForNextDay | missedCountForNextDay | message  | message1   | videoName | video          | title1                      | link                           | buttonText | image1      | question                       | option1  | option2 | title   | image    | count | resourceNumber |
        | admin | I         | K         | I         | 2         | E         | pdf  | pdf-test.pdf | 1        | 0           | 1             | 10:30 pm   | 3                  | 2                       | 1                     | textUser | testMesage | testVideo | shareVideo.mp4 | Download chrome for browser | https://www.google.com/chrome/ | OPENURL    | TestOne.jpg | Where are you joining us from? | New York | Toronto | NewFile | doll.jpg | 0     | 2              |
...
Scenario Outline: Check if it displays correct number of Registered,Attendee ,messages and missed in past session tab and under insights, visibility of graphs and start/end join time data under people tab
    ...
    Examples:
        | userA | eventName | videoName        | regEmail | regCount | launchTime | handoutTitle | image       | pollQuestion                        | optionOne | optionTwo | featureTitle | buttonText | image       | message                    | member    | role      | message       | featureUrl            | missedCount | attendeeCount | resourceNumber |
        | admin | K         | twoMinsVideo.mp4 | K        | 2        | 0h 0m 50s  | HandoutTest  | TestOne.jpg | Your favourite programming language | Java      | Python    | DemioLinks   | CLICK HERE | TestOne.jpg | Session has been scheduled | Subuser 1 | Moderator | Automatedtest | https://webdriver.io/ | 1           | 1             | 1              |
