Feature: Verify demio settings feature
  ...
  Scenario Outline: Check the functionality  of upload user profile
    Given User should be on Settings Page
    When User click on "Profile" tab
  ...
  Scenario Outline: Check for template Background image with custom domain
    ...
    Examples:
      | eventName | option           | image      | opcity | photo    | presenterName | presenterTitle        | presenterWebsite                   | presenterBio |
      | C         | Background Image | tweety.jpg | High   | doll.jpg | New Presenter | presenter for testing | my.staging.demio.com/manage/event/ | Tester Bio   |
