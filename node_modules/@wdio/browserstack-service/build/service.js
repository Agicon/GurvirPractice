...
class BrowserstackService {
    ...
    async onReload(oldSessionId, newSessionId) {
        ...
        if (!this.browser.isMultiremote) {
            ...
            const browserName = this.browser.instances.filter((browserName) => this.browser[browserName].sessionId === newSessionId)[0];
    ...
    _multiRemoteAction(action) {
        ...
        return Promise.all(this.browser.instances
            .filter(browserName => {
            const cap = util_1.getBrowserCapabilities(this.browser, this.caps, browserName);
            return util_1.isBrowserstackCapability(cap);
        })
            .map((browserName) => (action(this.browser[browserName].sessionId, browserName))));
    }
    ...
    async _printSessionURL() {
        await this._multiRemoteAction(async (sessionId, browserName) => {
            ...
            const capabilities = util_1.getBrowserCapabilities(this.browser, this.caps, browserName);
